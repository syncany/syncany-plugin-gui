...
class syncStateExtension(GObject.GObject, Nemo.ColumnProvider, Nemo.InfoProvider):
    ...
    def find_item_for_file(self, path):
        if path in self.NemoVFSFile_table:
            return self.NemoVFSFile_table[path]
        else:
            return None
    ...
    def invalidate_items_underneath(self, path):
        update_items = []
        if not self.NemoVFSFile_table:
	    self.askForOverlay(path)
	else:
	    for p in self.NemoVFSFile_table:
		if p == path or p.startswith(path):
		    item = self.NemoVFSFile_table[p]['item']
    ...
    def handle_server_response(self, l):
		...
		newState = parts[1]
			...
			if( not itemStore['state'] or newState != itemStore['state'] ):
			    ...
			    self.NemoVFSFile_table[parts[2]] = {'item': item, 'state':newState}
    ...
    def update_file_info(self, item):
        ...
        for reg_path in self.registered_paths:
            if filename.startswith(reg_path):
                self.NemoVFSFile_table[filename] = {'item': item, 'state':''}
